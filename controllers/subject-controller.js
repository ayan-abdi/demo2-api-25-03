...
const subjectController = {
    getAll: async (req, res) => {
        ...
        const { rows, count } = await db.subject.finAndCountAll({
    ...
    add: async (req, res) => {
        ...
        // Recuperation des données liées au login
    ...
    update: async (req, res) => {
        ...
        const [nbRow, updatedData] = await db.subject.update(data, { // la method update renvoie un tableau et donc on a fait un destructuring [ nbrow, updatedData]
            where: {
                [Op.and] : [
                    { id },   // id de l'element 
                    { memberId }  // id de l'element
                ]
            }, 
    ...
    delete: async (req, res) => {
        ...
        // la transaction est utile ici car le subject et en manytomany donc une association existe et reçoit les modifs
        const transaction = await db.sequelize.transaction();
    ...
    getAllMessage: async (req, res) => {
        ...
        const { rows, count } = await db.message.findAndCountAll({
            attributes: {
                exclude: ['subjectId']  // enleve moi le subjectId
            },
            where: { subjectId },
    ...
    addMessage: async (req, res) => {
        ...
        // Ajout  du memberid dans les données via JWT
        data.memberId = req.user.id;
